...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
 ...
 const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][2] = {
    [0] = {ENCODER_CCW_CW(KC_DOWN, KC_UP),             ENCODER_CCW_CW(KC_WH_D, KC_WH_U),                      ENCODER_CCW_CW(C(KC_LEFT), C(KC_RIGHT))      },
     ...
     [4] = {ENCODER_CCW_CW(KC_WH_L, KC_WH_R),           ENCODER_CCW_CW(KC_WH_D, KC_WH_U),                      ENCODER_CCW_CW(KC_VOLD,KC_VOLU)},
     [5] = {ENCODER_CCW_CW(RGB_VAD, RGB_VAI),           ENCODER_CCW_CW(RGB_SPD, RGB_SPI),                      ENCODER_CCW_CW(RGB_RMOD,RGB_MOD)},
     [6] = {ENCODER_CCW_CW(C(KC_LEFT), C(KC_RIGHT)),    ENCODER_CCW_CW(RCS(KC_TAB), C(KC_TAB)),                   ENCODER_CCW_CW(KC_WH_D, KC_WH_U)},
...
uint8_t get_layer_state(void) {
    ...
    uint8_t sw4 = gpio_read_pin(SW4_PIN) ? 0 : 1;
      ...
      // ONになっているスイッチの数をカウント
    uint8_t base_layer = sw1 + sw2 + sw3 + sw4;  // 0〜3
